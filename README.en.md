...
- **AVIF/AV1 decoding**: a pure MoonBit 8/10/12-bit 4:2:0 and monochrome path with shared entropy state across superblocks, multiple tiles, all 22 coding-block geometries with axes from 4 to 128 pixels, DCT/ADST residuals, transform-size selection and lossless 4×4 WHT. DC, zero-angle V/H, SMOOTH variants, PAETH and frame-level CDEF are supported. Original libaom output and constructed conformance streams use independently decoded dav1d native samples, followed by public RGBA entry checks; coverage includes sub-eight-pixel chroma ownership, tile seams and cropped edges. Monochrome and auxiliary alpha share this path; full-depth alpha uses rounded UNORM conversion checked against actual libavif output. CfL, nonzero angles, diagonal prediction, deblocking, loop restoration and general inter-frame syntax remain in development.
