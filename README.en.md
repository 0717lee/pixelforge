...
**MoonAV1 integration is deferred**: PixelForge continues using its built-in AV1/AVIF implementation and retains existing functionality. There is no active MoonAV1 dependency, snapshot or workspace wiring. The independent local MoonAV1 library is retained; dependency migration will resume after its release. The previous integration is reverted with a new commit, preserving Git history.
