...
- **AVIF/AV1 decoding**: a pure MoonBit 8/10/12-bit 4:2:0, 4:2:2, 4:4:4 and monochrome path with shared entropy state across superblocks, multiple tiles, all 22 coding-block geometries with axes from 4 to 128 pixels, DCT/ADST residuals, transform-size selection and lossless 4×4 WHT. DC, all eight directional modes with angle deltas and edge filtering/upsampling, SMOOTH variants, PAETH, all five filter-intra modes, 2–8-color palettes, CfL chroma prediction, native-depth deblocking and frame-level CDEF are supported. Normative horizontal super-resolution is decoded as well: use_superres and the coded denominator (nine through sixteen) keep entropy-coded and display widths distinct, and the fixed 8-tap phase convolution upscales complete native-depth planes after deblocking and CDEF, with CodedLossless and AllLossless staying distinct when scaling occurs. Deblocking precedes CDEF across tile boundaries. Original libaom output and constructed conformance streams use independently decoded dav1d native samples, followed by public RGBA entry checks; coverage includes sub-eight-pixel chroma ownership, tile seams, cropped edges, superres denominators 9–16 and the q0 screen-content/restoration syntax gates. Monochrome and auxiliary alpha share this path; full-depth alpha uses rounded UNORM conversion checked against actual libavif output. Loop restoration is fully decoded: frame-header syntax (per-plane lr_type codes and unit sizes), configuration validation and the unit-grid layout are threaded through every decode entry point, pre-superblock entropy reads fill a per-tile LR state, deblocked rows are snapshotted at internal stripe boundaries and supplied by the source view with top/bottom pairing and frame-edge replication, and the Wiener and SGR scalar filters run at native depth on the upscaled post-CDEF frame — pinned sample-by-sample against dav1d pixel ground truth (9539/9539 matching, 9429 of them changed by the filters). The general (non reduced-still) header grammar is now parsed — the full prefix with frame class, order hints, the seven-name reference list and its sign biases, `frame_size_with_refs`, motion-tool flags, skip mode and global motion — over an eight-slot native-depth reference buffer implementing `update_reference_map`, with the two-pass eight-tap subpel motion-compensation kernel in place (reference scaling, dual filters, narrow-block substitution and the 8/10/12-bit intermediate roundings). A key frame behind a general header is verified sample-by-sample against dav1d, and every asserted header field is checked against the FFmpeg `trace_headers` transcript. The reference-frame entropy-context snapshot/save/load is wired into the production decode path: a frame whose `primary_ref_frame` names a slot starts from that slot's stored entropy context, and `inter_cdf_inherit_64x64` now matches dav1d at [0, 0, 0] per plane. Stage C has unblocked `coded_lossless` (see the `inter_lossless_64x64` fixture), `is_motion_mode_switchable`/OBMC (both overlap-blend passes are implemented and separately covered, see the `inter_obmc_64x64` fixture), `allow_screen_content_tools` (palette syntax and index-map prediction for intra blocks inside an inter frame, see the `inter_palette_64x64` fixture) `allow_warped_motion`/the three-symbol `motion_mode` branch (`find_warp_samples` is implemented and separately covered, see the `inter_warped_64x64` fixture) and `use_ref_frame_mvs`/temporal motion vectors (`motion_field_estimation` now runs while the uncompressed header is read and `find_mv_stack` runs the §7.10.2.5 temporal scan between the near weighting and the corner scan, see the `inter_temporalmv_64x64` fixture; all five match dav1d sample-for-sample at [0,0,0]); the single-reference compound grammar (distance- and difference-weighted blends plus the interintra blend - see the `inter_compound_64x64`, `inter_distcomp_64x64`, `inter_diffwtd_64x64` and `inter_interintra_64x64` fixtures), skip mode over a three-frame sequence (see `inter_skipmode_64x64`) and translation global motion (see `inter_globalmv_64x64`) are unblocked as well. LOCALWARP itself is now sample-verified: `inter_localwarp_64x64` is the repository's first stream that actually selects `motion_mode=2` (produced by flipping one bit of `inter_warped_64x64`'s tile payload, because libaom never writes LOCALWARP for these groups), and its inter frame matches dav1d at [0, 0, 0] per plane with all 721 of its msac symbols carrying the same values and ranges as libdav1d. The rotate-zoom/affine global-motion model, also listed as refused before, is unblocked as well: the warped prediction grid is wired in, see the `inter_globalmv_rotzoom_64x64` fixture at [0,0,0] per plane. Segmentation is wired as far as an encoder will actually take it: the per-block `segment_id` symbol (between skip and the CDEF index, with `get_cur_frame_segid`'s neighbour context and `neg_deinterleave` for the coded difference) and the `SEG_LVL_ALT_Q` feature (the block's quantizer index, which feeds the dequantisation and the lossless decision) are both in the production path, and `inter_segmentation_64x64` - the repository's first stream that writes a segment map, encoded with `--aq-mode=2` - matches dav1d at [0, 0, 0] per plane on both its frames. The per-superblock quantizer delta of `delta_q_params` is in as well: `inter_deltaq_64x64` is the first stream that signals it (`--deltaq-mode=2`), each superblock origin codes one delta symbol that accumulates into that superblock's quantizer index, the residual dequantises per superblock while the coefficient CDF rows stay per tile exactly as libdav1d initialises them, and both frames match dav1d at [0, 0, 0]. Still refused: `delta_lf_present`, which needs a per-superblock loop-filter level table; a map update that carries no feature data of its own (the features would come from a primary reference); a temporal segment prediction (the map itself would come from the previous frame); and the other seven feature levels - the four loop-filter levels, the forced reference frame, the forced skip and the forced global motion. `inter_cdf_inherit_64x64` is a conformance-invalid stream (an 11-byte inter tile payload whose inherited decode over-reads by more than 1000 bits; libaom rejects it per spec 8.2 while dav1d 1.2.1 decodes it leniently) and is now closed at [0, 0, 0] per plane with inheritance enabled: the VERT/HORZ coefficient scan branches were swapped and the over-read guard removed (MSAC's `symbol_max_bits` is allowed to go negative and past-the-end reads supply the spec's padding zero bits, as libdav1d does). The sample stays in the suite as a regression fence; the diagnosis trail is in HANDOFF.md addendum 47.
- **AVIF grids**: `avif_decode` and `avif_decode_grid_auto` select the primary grid and its ordered `dimg` references, validate cell properties and dimensions, and compose native samples before color conversion. `avif_decode_rgba` also handles alpha grids and mixed grid/av01 primary/alpha pairs, retaining native alpha precision until final UNORM conversion.
