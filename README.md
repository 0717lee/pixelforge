...
- **AVIF 分阶段解析**：纯 MoonBit 校验 AVIF 主图项、`iloc` 数据范围、`av1C`/`ispe`/`nclx` 属性和 AV1 OBU/Reduced Still Sequence Header；`avif_stage1_input()` 已能从真实的 8-bit 4:2:0 无 alpha 单图中提取受限帧头后的 tile 字节，`av1_decode_dc_tile()` 已完成单 tile DC、单 DC 残差和全零色度残差的真实 MSAC 解码并生成 RGBA，其他预测模式与变换仍在后续阶段实现。
