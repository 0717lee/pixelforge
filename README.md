...
- **AVIF/AV1 解码**：纯 MoonBit 路径支持 8/10/12-bit 4:2:0、跨 superblock 的共享熵状态、单/多 tile、8–64 像素轴长的方形与矩形分块、DCT/ADST 残差、`tx_mode_select` 和 4×4 WHT 无损重建。已接入 DC、零角度增量的 V/H、SMOOTH 系列及 PAETH 预测，并以 45 个真实 libaom/dav1d 样本逐值验证原始位深 YUV 和最终 RGBA。非零角度、对角预测、帧级环路滤波及通用帧间语法仍在实现中。
