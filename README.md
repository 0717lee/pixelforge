...
- **AVIF/AV1 解码**：纯 MoonBit 路径支持 8/10/12-bit 4:2:0 与单色、跨 superblock 的共享熵状态、单/多 tile、8–128 像素轴长的方形与矩形分块、DCT/ADST 残差、`tx_mode_select` 和 4×4 WHT 无损重建。已接入 DC、零角度增量的 V/H、SMOOTH 系列及 PAETH 预测，以及跨 tile 的整帧 CDEF 方向搜索、强度选择和滤波。81 个真实 libaom/dav1d 样本逐值验证原始位深 YUV 和最终 RGBA，其中 CDEF 样本还覆盖 AVIF 容器、横纵 tile 接缝和奇数尺寸裁剪。单色灰度与辅助 alpha 已复用该路径，高位深透明度按 UNORM 四舍五入到 8 位，并由 3 个实际 alpha 容器核对。非零角度、对角预测、去块效应与环路恢复滤波及通用帧间语法仍在实现中。
