...
- **AVIF 分阶段解析**：纯 MoonBit 校验 AVIF 主图项、`iloc` 数据范围、辅助 alpha item（`auxl`/`auxC`）、`av1C`/`ispe`/`nclx` 属性和 AV1 OBU/Reduced Still Sequence Header；`avif_decode` 已支持受限单 tile 8-bit 4:2:0 DCT_DCT 像素重建，`av1_decode_alpha` 支持 64×64 monochrome alpha tile，四块平面重建和 8–12 bit YUV 转换已提供给后续管线；通用分区熵解码、alpha 合成、高位深 AV1 熵解码、网格和动画仍在后续阶段实现。
