...
- **AVIF 分阶段解析**：纯 MoonBit 校验 AVIF 主图项、`iloc` 数据范围、辅助 alpha item（`auxl`/`auxC`）、`av1C`/`ispe`/`nclx` 属性和 AV1 OBU/Reduced Still Sequence Header；`avif_stage1_input()` 已能从真实的 8-bit 4:2:0 无 alpha 单图中提取受限帧头后的 tile 字节，`av1_decode_dc_tile()` 已完成单 tile 的 square DCT_DCT 系数、DC/AC 残差、full-range YUV 与 RGBA 重建，通用块分区、方向预测、alpha 像素重建和高位深 AV1 熵解码仍在后续阶段实现。
